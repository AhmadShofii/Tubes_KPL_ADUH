...
import { 
  FiShoppingCart, FiSearch, FiMapPin, FiClock, 
  FiSend, FiStar, FiCamera, FiAtSign, FiMenu, FiX 
} from "react-icons/fi";
...
function Beranda() {
  ...
  // Variants untuk Animasi Menu Mobile (Diperbaiki agar mulus)
  const menuVariants = {
    closed: { opacity: 0, height: 0, overflow: "hidden", transition: { duration: 0.3 } },
    open: { opacity: 1, height: "auto", overflow: "hidden", transition: { duration: 0.4 } }
  };
  ...
  return (
    ...
    >
      {/* NAVBAR RESPONSIVE */}
      <motion.nav 
        className="navbar-beranda"
        initial={{ y: -80 }}
        animate={{ y: 0 }}
        transition={{ duration: 0.5, ease: "easeOut" }}
      >
        <Link to="/" className="logo-link">
          <h1 className="logo-beranda">Foodora</h1>
        </Link>

        {/* Desktop Links */}
        <div className="nav-links-beranda desktop-only">
          <Link to="/beranda" className="nav-item active">Home</Link>
          <Link to="/promo" className="nav-item">Promos</Link>
          <Link to="/history" className="nav-item">History</Link>
        </div>

        <div className="nav-right-beranda">
          <div className="icon-group">
            <FiShoppingCart className="nav-icon" />
            <FiMapPin className="nav-icon" />
          </div>
          
          <button className="btn-login-green desktop-only">Login</button>
          <button className="btn-register-green desktop-only">Register</button>
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div 
              ...
              exit="closed"
            >
              <Link to="/beranda" onClick={() => setIsMenuOpen(false)}>Home</Link>
              <Link to="/promo" onClick={() => setIsMenuOpen(false)}>Promos</Link>
